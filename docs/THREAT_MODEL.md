...
| A6 | **Compromised identity provider (Lichess)** | Controls what `lichess.org` returns to the OAuth exchange / account fetch, the game import stream, and the study-plan puzzle fetch (`GET /api/puzzle/next`). Treated like A4: nothing it returns is trusted beyond fail-closed-validated fields. Identity fields are validated to the username shape; imported games pin `evals=false`; puzzle positions are size-capped, replayed with python-chess and rejected unless `solution[0]` is a legal move, and are never a correctness oracle (the local engine judges every replay on `/training/verify-replay`). A hostile puzzle body can at worst waste a bounded number of fetches before the plan falls back to the local corpus. |

...
| 9 | Invented tactics | `validate_mode_2_semantic` | `engine_signal.tactical_flags == []` ⇒ text must NOT contain `fork` / `pin` / `sacrifice` / `attack` / `threat` ⇒ `Mode2Violation` | `test_fake_llm.py::test_compliant_output_passes` (smoke) | `violations.jsonl::SEM-02` | `test_llm_regression.py::test_llm_regression_contract` |

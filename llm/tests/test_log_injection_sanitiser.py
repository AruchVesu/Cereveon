...
engine_signal extractor, the events router, and the training router.
...
LOG-03  Untrusted HTTP request body fields (SolveRequest.source_type,
        SolveRequest.source_ref) reach logger calls in
        training/router.py when the dedup IntegrityError path fires.
        Same CWE-117 concern; closes CodeQL alerts #347–#348.
...
from llm.seca.training.router import _safe_log as _training_safe_log


_ALL_SANITISERS = [_engine_safe_log, _router_safe_log, _training_safe_log]


class TestSafeLogSanitiser(unittest.TestCase):
    """All sanitisers (one per module) must enforce the same contract.
    ...
    in a fourth module would obscure the sanitisation chain at the call
    site."""

    def test_strips_cr(self):
        for fn in _ALL_SANITISERS:
